...
class TraceHelper:
    ...
    def predict_episode(
    ...
    ) -> Tuple[Dict[str, float], Dict[str, torch.Tensor], list, list, list]:
        ...
        if measure_inference_time:
            stat_keys += ["infer_times", "n_windows"]
        ...
        for i in range(n_windows):
            ...
            window_elapsed_ms = (window_end_time - window_start_time) * 1000

            if "infer_times" in stat_keys:
                stats["infer_times"] += window_elapsed_ms
                stats["n_windows"] += 1
                print(f"Window {i+1}/{n_windows} inference time: {window_elapsed_ms:.3f} ms")
            ...
            # debug #
            # plt.imshow(ret["mask"][i_from:i_to][:, ::6].numpy(), cmap='binary')
            # plt.title('Binary Mask Visualization')
            # plt.colorbar()
            # plt.savefig(f'mask_visualization_window_{i}.png')
            # plt.close()
            # mr = ret["mask"][i_from:i_to][:, ::6].sum() / ret["mask"][i_from:i_to][:, ::6].reshape(-1).shape[0]

            
            pred = reshape_tensor(ret['dap_b'][i_from:i_to], upscale=True, sports=sports, single_team=self.params["single_team"], mode="pos")
            target = reshape_tensor(ret['target'][i_from:i_to], upscale=True, sports=sports, single_team=self.params["single_team"], mode="pos")
            mask = reshape_tensor(ret['mask'][i_from:i_to], sports=sports, single_team=self.params["single_team"], mode="pos")

            lambdas = ret["lambdas"][i_from:i_to].reshape(window_ret["target"].shape[1], -1, 3) # [time, players, 3]

            segments, pe, lambdas = TraceHelper.get_missing_segments(mask, pred, target, lambdas)
            missing_segments.extend(segments)
            segment_pe.extend(pe)
            segment_lambdas.extend(lambdas)
            # debug #
        ...
        return ret, stats, missing_segments, segment_pe, segment_lambdas
    ...
    def predict(
        ...
        for phase in self.traces["phase"].unique():
            ...
            for e in tqdm(episodes, desc=f"Phase {phase}"):
                ...
                with torch.no_grad():
                    ep_ret, ep_stats, missing_segments, segment_pe, segment_lambdas = self.predict_episode(
                        model,
                        sports,
                        ep_player_traces,
                        ep_ball_traces,
                        ...
                        segment_pe=segment_pe,
                        segment_lambdas=segment_lambdas,
    ...
    @staticmethod
    def get_missing_segments(mask: torch.Tensor, pred: torch.Tensor, target: torch.Tensor, ensemble_weights: torch.Tensor):
        '''
        mask: [time, players, 2]
        pred: [time, players, 2]
        target: [time, players, 2]
        ensemble_weights: [time, players, 3]
        '''
        segments = []
        pe = []
        ensemble_w = []
        for p in range(mask.shape[1]):
            ...
            for i in range(mask.shape[0]):
                ...
                if is_missing and not in_segment:
                    ...
                    segment_pred = pred[start_idx:i-1, p] # [time, 2]
                    segment_target = target[start_idx:i-1, p]

                    segment_pe = torch.norm((segment_pred - segment_target), dim=-1).mean()
                    pe.append(segment_pe)
                    ensemble_w.append(ensemble_weights[start_idx:i-1, p])

            # Handle case where segment ends at last frame
            if in_segment:
                segments.append((start_idx, mask.shape[0] - 1))
                segment_pred = pred[start_idx:mask.shape[0]-1, p] # [time, 2]
                segment_target = target[start_idx:mask.shape[0]-1, p]

                segment_pe = torch.norm((segment_pred - segment_target), dim=-1).mean()
                pe.append(segment_pe)
                ensemble_w.append(ensemble_weights[start_idx:mask.shape[0]-1, p])

        return segments, pe, ensemble_w
