    ...
    def plot_trajectories(self, seq):
        ...
        for i, (key, title) in enumerate(self.pred_keys.items()):
            ...
            for i, k in enumerate(range(n_players)):
                ...
                #     # 그림 저장
                #     fig_ind.savefig(f"individual_players/epi_{self.epi_idx}/seq_{seq}_player_{k}_{segment_class.lower()}.png", 
                ...
                #     plt.close(fig_ind)
                    ...
                    #         fig_weights = plt.figure(figsize=(1.1, 1.1), dpi=300, facecolor='white')
                            
                    #         # 단일 axes 생성 - 궤적 figure와 유사한 여백 설정
                    #         ax = fig_weights.add_axes([0.05, 0.05, 0.9, 0.9])
                    ...
                    #         # 틱 위치 및 레이블 설정 - 텍스트 크기를 궤적 figure와 동일하게 설정
                    #         ax.set_yticks([0.5, 2.5, 4.5])
                    #         ax.set_yticklabels(["IP", "DAP-F", "DAP-B"], fontsize=7)  # 궤적 figure와 동일한 텍스트 크기
                    ...
                    #         cbar.ax.tick_params(labelsize=7)  # 궤적 figure와 동일한 텍스트 크기
                            
                    #         # 저장 옵션도 궤적 figure와 동일하게 설정
                    #         fig_weights.savefig(f"individual_players/epi_{self.epi_idx}/seq_{seq}_player_{k}_{segment_class.lower()}_weights.png", 
                    #                           bbox_inches="tight", pad_inches=0.02, transparent=False)
