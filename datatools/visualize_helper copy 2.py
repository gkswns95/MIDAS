...
class VisualizeHelper:
    ...
    def plot_trajectories(self, seq):
        ...
        for i, (key, title) in enumerate(self.pred_keys.items()):
            ...
            for i, k in enumerate(range(n_players // 2)):
                ...
                # === 선수별 개별 figure 저장 ===
                if key == "dbhp":
                    os.makedirs("individual_players", exist_ok=True)
                    ...
                    # 액자 스타일 figure 생성
                    fig_ind = plt.figure(figsize=(1.1, 1.1), dpi=300, facecolor='white')
                    ax_ind = fig_ind.add_axes([0.05, 0.05, 0.9, 0.9])  # 테두리를 위한 여백 추가
                    
                    # 배경 설정
                    ax_ind.set_facecolor('white')
                    
                    # 범례를 위한 라벨 추가
                    gt_line = ax_ind.plot(target_x, target_y, color=c[i], linewidth=0.7, alpha=0.5,
                                     label='Ground Truth', markersize=0)[0]  # markevery로 legend 길이 조절
                    
                    # 결측 구간만 선명하게 표시 - 점선 패턴 수정
                    imp_line = ax_ind.plot(x[missing_idxs], y[missing_idxs], 
                                    color=c[i], linewidth=1.2, linestyle='--', 
                                    dashes=[2, 1.5],  # 짧은 대시, 짧은 간격으로 수정
                                    label='Imputed')[0]
                    
                    # 시작점 표시
                    ax_ind.scatter(x[0], y[0], color=c[i], s=10, marker='o', edgecolor="black", linewidths=0.5)
                    ...
                    max_range = max(x_range, y_range) * 1.1  # 여백 추가
                    ax_ind.set_xlim(x_center - max_range/2, x_center + max_range/2)
                    ax_ind.set_ylim(y_center - max_range/2, y_center + max_range/2)
                    
                    # 범례 추가 (테두리 없음, 길이 줄임)
                    legend = ax_ind.legend(handles=[gt_line, imp_line], 
                                        labels=['Ground Truth', 'Imputed'],
                                        loc='upper right', fontsize=5, 
                                        frameon=False,  # 테두리 비활성화
                                        handlelength=1.9,  # 핸들 길이 줄임 (점선 갯수 줄이기)
                                        handletextpad=0.5,
                                        borderaxespad=0.1,
                                        handler_map={imp_line: DashedLineHandler(), gt_line: HandlerLine2D(numpoints=1)}
                                        )
                    
                    # 액자 스타일 테두리 추가 (Rectangle 패치 사용)
                    rect = Rectangle( 
                        xy=(ax_ind.get_xlim()[0], ax_ind.get_ylim()[0]),
                        width=ax_ind.get_xlim()[1] - ax_ind.get_xlim()[0],
                        height=ax_ind.get_ylim()[1] - ax_ind.get_ylim()[0],
                        fill=False,
                        edgecolor='black',
                        linewidth=2,
                        zorder=10
                    ...
                    # 그림 저장
                    fig_ind.savefig(f"individual_players/seq_{seq}_player_{k}_{segment_class.lower()}.png", 
                    ...
                    if "window_lambdas" in self.pred_dict:
                        ...
                        if len(missing_idxs) > 0:
                            ...
                            # 3개 방법에 대한 결측 구간의 가중치 추출 - 순서 변경 (첨부 이미지와 동일하게 DP가 아래로)
                            ...
                            nan_row = np.full_like(dp_weights, np.nan)  # NaN으로 채워진 행 생성
                            ...
                            # seaborn heatmap 생성 - 빈 레이블에 틱 표시 안 함
                            fig_weights = plt.figure(figsize=(3, 3), dpi=300, facecolor='white')
                            ...
                            sns.heatmap(weights_matrix, cmap="viridis", 
                                      cbar=True, 
                                      xticklabels=False, 
                                      yticklabels=False,  # 일단 모든 yticklabels 비활성화
                                      vmin=0, vmax=1, ax=ax,
                                      mask=np.isnan(weights_matrix))  # NaN 값은 마스킹하여 표시 안 함
                            
                            # 틱 위치를 직접 설정 (데이터가 있는 행의 중앙 위치에만 틱 설정)
                            ax.set_yticks([0.5, 2.5, 4.5])  # 데이터 행의 중앙 위치
                            ax.set_yticklabels(["IP", "DAP-F", "DAP-B"])  # 각 데이터 행에 대한 레이블
                            
                            # x축 레이블 추가
                            # ax.set_xlabel("Missing Segment Position")
                            ax.set_xticks([0, segment_length-1])
                            ax.set_xticklabels(["missing start", "missing end"])
                            
                            # 제목 추가
                            # ax.set_title(f"Player {k} Hybrid Weights (Length: {segment_length})")
                            ...
                            # 저장
                            fig_weights.savefig(f"individual_players/seq_{seq}_player_{k}_{segment_class.lower()}_weights.png", 
    ...
    def plot_run(self, epi_idx):
        ...
        for seq, i in enumerate(range(epi_traces.shape[0] // self.wlen + 1)):
            ...
            # self.fig.savefig(f"{path}/seq_{seq}_target_imputed.pdf", bbox_inches="tight", pad_inches=0.009)
